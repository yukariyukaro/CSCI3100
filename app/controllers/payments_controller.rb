class PaymentsController < ApplicationController
  ...
  def create
    transaction = current_community_scope(Transaction).includes(:product).find(params[:transaction_id])
  ...
  def fake
    @payment = payment_scope.includes(product_transaction: :product).find(params[:id])
    head :forbidden unless fake_checkout_allowed?(@payment)
  end
  ...
  def redirect_not_payable(transaction)
    redirect_to product_path_for(transaction.product), alert: t("payments.errors.not_payable")
  end
  ...
  def settle_and_respond(payment, payload)
    ...
    redirect_to product_path_for(payment.product_transaction.product), notice: t("payments.success")
  end
  ...
  def webhook_cancel(payment)
    ...
    if provider.name == "fake"
      redirect_to product_path_for(payment.product_transaction.product), alert: t("payments.cancelled")
    else
      head :ok
    end
  end
  ...
  def product_path_for(product)
    community_product_path(community_slug: product.community.slug, id: product)
  end

  def fake_checkout_allowed?(payment)
    payment.provider == "fake" &&
      payment.product_transaction.buyer_id == current_user.id &&
      token_matches?(params[:token], payment.callback_token)
  end
end
